...
class TopicsViewModel {
    ...
    fileprivate func fetchTopicsAndReloadUI() {
        topicsDataManager.fetchAllTopics { [weak self] (result) in
            ...
            case .success(let response):
                ...
                self?.topicViewModels.insert(HeaderViewCellModel(), at: 0)
    ...
    func didSelectRow(at indexPath: IndexPath) {
        ...
        let topicCellViewModel = topicViewModels[indexPath.row] as? TopicCellViewModel
        guard let topic = topicCellViewModel?.topic else { return  }
